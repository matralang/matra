...
function renderMatrastNode([tag, props, body]) {
    ...
    switch (tag) {
        case "code": {
            ...
            return `<pre><code class="lang-${escapeAttr(lang)}">${escapeHTML(content)}</code></pre>`;
        }
        case "a":
            return `<a href="${escapeAttr(props.href)}">${toHTML(body)}</a>`;
        case "img":
            return `<img src="${escapeAttr(props.src)}" alt="${escapeAttr(props.alt || "")}" />`;
        case "tagdef":
            return `<!-- tagdef ${escapeHTML(props.name)} (${Array.isArray(body) ? body.length : 0} fields) -->`;
        default: {
            // Regular HTML tags
...
export function toTeX(node) {
    ...
    if (Array.isArray(node)) {
        ...
        if (typeof node[0] === "string" && node.length <= 3) {
            ...
            switch (tag) {
                case "h1":
                    return `\\section*{${Array.isArray(body) ? body.map(toTeX).join("") : body}}`;
                case "h2":
                    return `\\subsection*{${Array.isArray(body) ? body.map(toTeX).join("") : body}}`;
                case "h3":
                    return `\\subsubsection*{${Array.isArray(body) ? body.map(toTeX).join("") : body}}`;
                case "p":
                ...
                case "a":
                    return `\\href{${props.href}}{${Array.isArray(body) ? body.map(toTeX).join("") : body}}`;
                default:
